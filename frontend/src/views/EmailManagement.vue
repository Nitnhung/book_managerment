...
onMounted(() => {
  ...
  if (savedHistory) {
    try {
      emailHistory.value = JSON.parse(savedHistory)
    } catch (e) {
      console.error('Không thể load lịch sử email', e)
    }
  }
})
...
async function sendReminderEmails() {
  ...
  try {
    ...
    messageType.value = res.data.failedCount && res.data.failedCount > 0 ? 'error' : 'success'

    const newEntries = []
    
    if (Array.isArray(res.data.successes)) {
      res.data.successes.forEach(s => {
        newEntries.push({
    ...
    if (newEntries.length > 0) {
      // Prepend new entries and assign unique IDs based on current length to avoid collision
      const startingId = emailHistory.value.length
      const formattedEntries = newEntries.map((item, idx) => ({ ...item, id: startingId + idx }))
      
      emailHistory.value = [...formattedEntries, ...emailHistory.value]
